...
namespace E_commerce_Stage2
{
    ...
    {
        private List<IProductDetails> products; // Stores products
        private int idCounter;          // Keeps track of next unique ID

        public InventoryManager()//Initialize Product Details
        {
            products = new List<IProductDetails>();
            idCounter = 1;
        }

        /// Add a new product to inventory based on type.
        public IProductDetails AddProduct(string type, string name, decimal price, string specialProperty)
        {
            IProductDetails newProduct = type.ToLower() switch
            {
        ...
        public List<IProductDetails> GetAllProducts() => new List<IProductDetails>(products);

        /// Find a product by ID or returns null.
        public IProductDetails GetProductById(int id) => products.FirstOrDefault(p => p.Id == id);

        /// Update an existing product's details.
        public bool UpdateProduct(int id, string newName, decimal newPrice, string newExtraProperty)
        {
            IProductDetails product = GetProductById(id);
